...
test("配置工程与 S0 显式豁免，但业务包仍不能依赖 S0", () => {
  withWorkspace(
    ["apps/*", "packages/*", "tools/s0"],
    ({ root, write, pkg }) => {
      pkg("apps/admin", "admin", {
        devDependencies: { "@workspace/eslint-config": "workspace:*" },
      })
      pkg("packages/eslint-config", "@workspace/eslint-config")
      pkg("packages/typescript-config", "@workspace/typescript-config")
      pkg("tools/s0", "@workspace/s0", {
        dependencies: { "drizzle-orm": "1.0.0" },
      })
      assert.deepEqual(checkBoundaries(root), [])
      write("tools/testing/helper.mjs", "export {}")
      write(
        "apps/admin/src/index.ts",
        'import "../../../tools/testing/helper.mjs"'
      )
      assert.deepEqual(checkBoundaries(root), [])
      write("apps/admin/src/index.ts", 'import "@workspace/s0"')
      assert.equal(checkBoundaries(root).length, 1)
      write("apps/admin/src/index.ts", 'import "../../../tools/s0/src/index"')
      assert.equal(checkBoundaries(root).length, 1)
    }
  )
})
