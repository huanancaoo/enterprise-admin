...
test(
  ...
  async (t) => {
    ...
    // 按依赖顺序关闭：先 HTTP，再连接池，最后容器，避免强制断开仍存活的 pg 连接。
    t.after(async () => {
      await closeApp?.()
      await pool?.end()
      await owner.end()
    ...
    const owner = new Pool({ ...config, user: "postgres", password })
    ...
    pool = runtime
    const { app, document, url, auth } = await startProbe((baseURL) =>
    ...
    await t.test(
      "所有认证组织引用生成 UUID，业务外键及 RLS cast 使用同一类型",
      async () => {
        const columns =
          await owner.query(`SELECT table_name, column_name, data_type FROM information_schema.columns
      ...
      OR column_name IN ('organization_id', 'active_organization_id'))`)
        ...
        await owner.query(`CREATE POLICY uuid_probe_scope ON uuid_probe TO s0_runtime
    ...
    await t.test(
      "合法/非法 Body、UUID Path 和 Query coercion 经过真实 HTTP 校验",
      async () => {
    ...
    await t.test(
      "同一个 Zod 字段进入 OpenAPI 和可执行的 Orval SDK",
      async () => {
