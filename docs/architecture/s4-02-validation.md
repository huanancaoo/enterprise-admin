...
`test:e2e` 已纳入 `pnpm verify`，因此 CI 包含这些浏览器流程。
...
| 检查           | 结果                                                                                      |
| -------------- | ----------------------------------------------------------------------------------------- |
| `pnpm verify`  | 通过：lint、类型检查、单元 7 项、API 8 项、Storybook 10 项、浏览器 E2E 5 项、全部应用构建 |
| 数据库专项检查 | 通过：认证 Schema 无漂移，数据库与租户隔离 13 项                                          |
...
- `pnpm verify`：首步 peer 检查失败。`@tiptap/extension-bubble-menu` 与 `@tiptap/extension-floating-menu` 为 3.31.3，要求 3.31.3 的 core/pm，而现有 core/pm 固定为 3.30.3。该组合已存在于基线 `3f9af15` lockfile。
  该阻塞已在 2026-09-15 后续工作中关闭：Tiptap 依赖统一为 3.31.3，当前 `pnpm peers check` 和根目录 `pnpm verify` 均已通过。本节保留原始验收时点，避免把后续修复倒记为 S4-02 当时的结果。
