...
- 根 `pnpm verify` 的数据库测试自动覆盖 S3；新增 `pnpm verify:s3` 与 `pnpm test:isolation` 供独立复现。
...
- `pnpm verify:s3` 的边界、类型、lint 和数据库测试通过；后续新增的跨组织 UPDATE/DELETE 用例也在完整 `pnpm verify` 中通过。
- 认证 Schema 重生成无漂移。
